...
async function setupDatabase() {
  ...
  // 1. Save admin user data before dropping tables
  let adminData = null
  try {
    const r = await pool.query(
      `SELECT id, code, username, email, password_hash, avatar, created_at
       FROM users WHERE email=$1`, [ADMIN_EMAIL]
    )
    if (r.rows.length > 0) adminData = r.rows[0]
  } catch { /* table may not exist yet */ }

  // 2. Drop ALL tables (except users temporarily) and recreate with correct schemas
  const dropOrder = [
  ...
  // 4. Recreate ALL tables with correct schemas
  const schemas = {
    users: `
  ...
  for (const [table, sql] of Object.entries(schemas)) {
    try {
      ...
      if (err.code !== '42P07') console.error(`Schema error [${table}]:`, err.message)
    }
  }
  ...
  try {
    ...
    if (adminData) {
      ...
      await pool.query(
        ...
        [adminData.code, adminData.username, adminData.email, adminHash, null, ADMIN_BALANCE, adminData.created_at]
      )
    } else {
      await pool.query(
        `INSERT INTO users (code, username, email, password_hash, role, avatar, is_verified, is_active, profile_completed, balance, credits)
         ...
         ON CONFLICT (email) DO UPDATE SET
           password_hash=$4, role='administrador', is_verified=TRUE, profile_completed=TRUE, balance=$5, credits=$5`,
